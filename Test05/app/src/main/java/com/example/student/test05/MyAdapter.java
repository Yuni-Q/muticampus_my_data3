...
public class MyAdapter extends ArrayAdapter<ContactVO> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        // 데이터를 아이템으로 만들어서 리스트로 보내는 역활 !!!

        //inflate는 xml을 한꺼번에 팽창시키는 도구
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(context.LAYOUT_INFLATER_SERVICE);
        View wholeItem = inflater.inflate(layoutId, null); // 팽차!!

        TextView textViewName = (TextView) wholeItem.findViewById(R.id.text_name);
        TextView textViewNumber = (TextView) wholeItem.findViewById(R.id.text_number);
        Button btnCall = (Button) wholeItem.findViewById(R.id.btn_call);
        Button btnSms = (Button) wholeItem.findViewById(R.id.btn_sms);
        ...
        btnCall.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (ActivityCompat.checkSelfPermission(context, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                    // 만약 폰주인이 이 앱의 전화걸기 권한 꺼놓은 경우 어떻게 할지 적는 조건문
                    return;
                }
                context.startActivity(intent);// 전화걸기 permission
        ...
        btnSms.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(context,"문자?",Toast.LENGTH_SHORT).show();
            }
        });
